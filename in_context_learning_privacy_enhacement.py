...
with open(output_csv_file, newline='', encoding='utf-8') as csvfile:
    ...
    for idx, row in enumerate(reader, start = 1):
        ...
        if (idx) % 3 == 0:
            ...
            def encode_all_casings(tokenizer, phrase: str):
                """Return token‑id sequences for several common casings of *phrase*."""
                variants = {
                    phrase,
                    phrase.lower(),
                    phrase.upper(),
                    phrase.title(),        # e.g. “Alex Adams”
                    phrase.capitalize(),   # e.g. “Alex adams”
                }
                return [tokenizer.encode(v, add_special_tokens=False) for v in variants]
            ...
            banned_phrases = [v.strip() for v in extracted_values if v.strip()]
            #bad_words_ids = [tokenizer.encode(p, add_special_tokens=False)
                             #for p in set(banned_phrases)      
            #]
            
            bad_words_ids  = []

            for p in set(banned_phrases):
                bad_words_ids.extend(encode_all_casings(tokenizer, p))
            ...
            if any(result_dict.values()):     # True if at least one entity leaked
                privacy_percentage += 1
                print(f"Privacy leaked: {result_dict}")
            total += 1
...
out_path = "synthetic_ft_data_icl_privacy.jsonl"
